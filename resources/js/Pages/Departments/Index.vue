      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $t('Departments Management') }}</h1>
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $t('Manage organizational departments') }}</p>
            </div>
            <Link
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
              <tr v-for="department in departments.data" :key="department.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    <div class="w-8 h-8 bg-green-100 dark:bg-green-900/30 rounded-lg flex items-center justify-center mr-3">
                      <Building class="w-4 h-4 text-green-600 dark:text-green-400" />
                    </div>
                    <div>
                      <div class="text-sm font-medium text-slate-900 dark:text-white">{{ department.name }}</div>
                    </div>
                  </div>
                </td>
              ...
              <tr v-if="departments.data.length === 0">
                <td colspan="3" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center">
                    <Building class="w-12 h-12 text-slate-400 dark:text-slate-500 mb-4" />
                    <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">{{ $t('No departments found') }}</h3>
                    <p class="text-slate-500 dark:text-slate-400 mb-4">{{ $t('Get started by creating your first department') }}</p>
                    <Link
                      :href="route('departments.create')"
                      class="inline-flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition-colors duration-200 font-medium"
                    >
                      <Plus class="w-4 h-4" />
                      {{ $t('Create Department') }}
                    </Link>
