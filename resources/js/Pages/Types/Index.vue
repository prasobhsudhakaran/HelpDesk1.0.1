      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="py-6">
          <div class="flex items-center justify-between">
            <div>
              <h1 class="text-2xl font-bold text-slate-900 dark:text-white">{{ $t('Types Management') }}</h1>
              <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">{{ $t('Manage ticket types and classifications') }}</p>
            </div>
            <Link
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-700">
              <tr v-for="type in types.data" :key="type.id" class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition-colors duration-150">
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="flex items-center">
                    ...
                    <div>
                      <div class="text-sm font-medium text-slate-900 dark:text-white">{{ type.name }}</div>
                    </div>
                  </div>
                </td>
              ...
              <tr v-if="types.data.length === 0">
                <td colspan="3" class="px-6 py-12 text-center">
                  <div class="flex flex-col items-center">
                    <FileText class="w-12 h-12 text-slate-400 dark:text-slate-500 mb-4" />
                    <h3 class="text-lg font-medium text-slate-900 dark:text-white mb-2">{{ $t('No types found') }}</h3>
                    <p class="text-slate-500 dark:text-slate-400 mb-4">{{ $t('Get started by creating your first type') }}</p>
                    <Link
                      :href="route('types.create')"
                      class="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors duration-200 font-medium"
                    >
                      <Plus class="w-4 h-4" />
                      {{ $t('Create Type') }}
                    </Link>
  ...
  methods: {
    ...
    deleteType(type) {
      this.showDeleteConfirmation({
        ...
        itemName: type.name,
        itemType: 'type',
        deleteUrl: route('types.destroy', type.id),
        deleteMethod: 'delete'
      });
    },
    formatDate(date) {
