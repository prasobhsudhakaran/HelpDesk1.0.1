...
import { resolveComponent, withCtx, createVNode, toDisplayString, createBlock, createCommentVNode, createTextVNode, openBlock, useSSRContext } from "vue";
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  ssrRenderList($props.users.data, (user) => {
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(user.city || "-")}`);
        } else {
          return [
            createTextVNode(toDisplayString(user.city || "-"), 1)
          ];
        }
      }),
      _: 2
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate($options.formatDate(user.created_at))}`);
        } else {
          return [
            createTextVNode(toDisplayString($options.formatDate(user.created_at)), 1)
          ];
        }
      }),
      _: 2
