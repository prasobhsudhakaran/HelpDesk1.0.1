...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  ssrRenderList($props.types.data, (type) => {
    _push(`<tr class="hover:bg-gray-100 focus-within:bg-gray-100"><td class="border-t">`);
    _push(ssrRenderComponent(_component_Link, {
      class: "px-6 py-4 flex items-center focus:text-indigo-500",
      href: this.route("types.edit", type.id)
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(type.name)}`);
        } else {
          return [
  ...
  _push(`<!--]-->`);
  if ($props.types.data.length === 0) {
    _push(`<tr><td class="border-t px-6 py-4" colspan="4">No types found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Types/Index.vue");
