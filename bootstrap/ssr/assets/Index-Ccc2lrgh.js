...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(ssrRenderComponent(_component_Link, {
    class: "btn-indigo",
    href: this.route("departments.create")
  }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<span${_scopeId}>${ssrInterpolate(_ctx.$t("Create"))}</span><span class="hidden md:inline"${_scopeId}> ${ssrInterpolate(_ctx.$t("Department"))}</span>`);
      } else {
        return [
          createVNode("span", null, toDisplayString(_ctx.$t("Create")), 1),
          createVNode("span", { class: "hidden md:inline" }, " " + toDisplayString(_ctx.$t("Department")), 1)
        ];
      }
    }),
    _: 1
  ...
  ssrRenderList($props.departments.data, (department) => {
    _push(`<tr class="hover:bg-gray-100 focus-within:bg-gray-100"><td class="border-t">`);
    _push(ssrRenderComponent(_component_Link, {
      class: "px-6 py-4 flex items-center focus:text-indigo-500",
      href: this.route("departments.edit", department.id)
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
  ...
  _push(`<!--]-->`);
  if ($props.departments.data.length === 0) {
    _push(`<tr><td class="border-t px-6 py-4" colspan="4">No departments found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Departments/Index.vue");
