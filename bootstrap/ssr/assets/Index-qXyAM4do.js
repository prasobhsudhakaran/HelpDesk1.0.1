...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(ssrRenderComponent(_component_Link, {
    class: "btn-indigo",
    href: this.route("statuses.create")
  }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<span${_scopeId}>${ssrInterpolate(_ctx.$t("Create"))}</span><span class="hidden md:inline"${_scopeId}> ${ssrInterpolate(_ctx.$t("Status"))}</span>`);
      } else {
        return [
          createVNode("span", null, toDisplayString(_ctx.$t("Create")), 1),
          createVNode("span", { class: "hidden md:inline" }, " " + toDisplayString(_ctx.$t("Status")), 1)
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div><div class="bg-white rounded-md shadow overflow-x-auto"><table class="w-full whitespace-nowrap"><tbody><tr class="text-left font-bold"><th class="px-6 pt-6 pb-4">${ssrInterpolate(_ctx.$t("Name"))}</th><th class="px-6 pt-6 pb-4">${ssrInterpolate(_ctx.$t("Slug"))}</th></tr><!--[-->`);
  ssrRenderList($props.statuses.data, (status) => {
    _push(`<tr class="hover:bg-gray-100 focus-within:bg-gray-100"><td class="border-t">`);
    _push(ssrRenderComponent(_component_Link, {
      class: "px-6 py-4 flex items-center focus:text-indigo-500",
      href: this.route("statuses.edit", status.id)
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(status.name)}`);
        } else {
          return [
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(status.slug)}`);
        } else {
          return [
            createTextVNode(toDisplayString(status.slug), 1)
          ];
        }
      }),
      _: 2
  ...
  _push(`<!--]-->`);
  if ($props.statuses.data.length === 0) {
    _push(`<tr><td class="border-t px-6 py-4" colspan="4">No statuses found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
...
_sfc_main.setup = (props, ctx) => {
  ...
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/Statuses/Index.vue");
