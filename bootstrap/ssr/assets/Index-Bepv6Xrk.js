...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(ssrRenderComponent(_component_Link, {
    class: "btn-indigo",
    href: this.route("roles.create")
  }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<span${_scopeId}>${ssrInterpolate(_ctx.$t("Create a New Role"))}</span>`);
      } else {
        return [
          createVNode("span", null, toDisplayString(_ctx.$t("Create a New Role")), 1)
        ];
      }
    }),
    _: 1
  }, _parent));
  _push(`</div><div class="bg-white rounded-md shadow overflow-x-auto"><table class="w-full whitespace-nowrap"><tbody><tr class="text-left font-bold"><th class="px-6 pt-6 pb-4">#${ssrInterpolate(_ctx.$t("ID"))}</th><th class="px-6 pt-6 pb-4">${ssrInterpolate(_ctx.$t("Name"))}</th><th class="px-6 pt-6 pb-4">${ssrInterpolate(_ctx.$t("Slug"))}</th></tr><!--[-->`);
  ssrRenderList($props.roles.data, (role) => {
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      href: this.route("roles.edit", role.id)
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(role.id)}`);
        } else {
          return [
            createTextVNode(toDisplayString(role.id), 1)
          ];
        }
      }),
      _: 2
    ...
    _push(ssrRenderComponent(_component_Link, {
      ...
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(role.slug)}`);
        } else {
          return [
            createTextVNode(toDisplayString(role.slug), 1)
          ];
        }
      }),
      _: 2
  ...
  if ($props.roles.data.length === 0) {
    _push(`<tr><td class="border-t px-6 py-4" colspan="4">No roles found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
