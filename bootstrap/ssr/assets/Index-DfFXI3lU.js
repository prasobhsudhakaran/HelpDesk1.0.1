...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<div${ssrRenderAttrs(_attrs)}><h1 class="mb-8 font-bold text-3xl">${ssrInterpolate(_ctx.$t("City"))}</h1><div class="mb-6 flex justify-between items-center">`);
  ...
  _push(ssrRenderComponent(_component_Link, {
    class: "btn-indigo",
    href: this.route("cities.create")
  }, {
    default: withCtx((_, _push2, _parent2, _scopeId) => {
      if (_push2) {
        _push2(`<span${_scopeId}>${ssrInterpolate(_ctx.$t("Create"))}</span>  <span class="hidden md:inline"${_scopeId}>${ssrInterpolate(_ctx.$t("City"))}</span>`);
      } else {
        return [
          createVNode("span", null, toDisplayString(_ctx.$t("Create")), 1),
          createTextVNode("  "),
          createVNode("span", { class: "hidden md:inline" }, toDisplayString(_ctx.$t("City")), 1)
        ];
      }
    }),
    _: 1
  ...
  ssrRenderList($props.cities.data, (city) => {
    _push(`<tr class="hover:bg-gray-100 focus-within:bg-gray-100"><td class="border-t">`);
    _push(ssrRenderComponent(_component_Link, {
      class: "px-6 py-4 flex items-center focus:text-indigo-500",
      href: this.route("cities.edit", city.id)
    }, {
      default: withCtx((_, _push2, _parent2, _scopeId) => {
        if (_push2) {
          _push2(`${ssrInterpolate(city.name)}`);
        } else {
          return [
  ...
  if ($props.cities.data.length === 0) {
    _push(`<tr><td class="border-t px-6 py-4" colspan="4">No cities found.</td></tr>`);
  } else {
    _push(`<!---->`);
  }
