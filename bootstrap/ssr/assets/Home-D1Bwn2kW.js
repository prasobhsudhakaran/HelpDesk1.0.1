...
import { T as TextareaInput } from "./TextareaInput-B38GbHxw.js";
import axios from "axios";
import { resolveComponent, withCtx, createBlock, openBlock, Fragment, renderList, toDisplayString, createTextVNode, useSSRContext } from "vue";
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`<!--]--></div><div class="${ssrRenderClass([{ "active": $data.tabs[2].active }, "p-4 t-content"])}"><div class="flex flex-col items-start gap-[30px] pl-5 pt-5 w-full lg:w-1/3"><label class="flex toggle_swtich items-center cursor-pointer"><div class="relative"><input type="checkbox" class="sr-only"${ssrIncludeBooleanAttr(Array.isArray($data.form.html.sections[2].enable_ticket_section) ? ssrLooseContain($data.form.html.sections[2].enable_ticket_section, null) : $data.form.html.sections[2].enable_ticket_section) ? " checked" : ""}><div class="w-10 h-4 bg-gray-400 rounded-full shadow-inner"></div><div class="dot absolute w-6 h-6 bg-white rounded-full shadow -left-1 -top-1 transition"></div></div><div class="ml-3 text-sm">${ssrInterpolate("Enable " + _ctx.$t("Ticket") + " Section")}</div></label></div></div></div></div></div><div class="px-8 py-4 bg-gray-50 border-t border-gray-100 flex items-center">`);
